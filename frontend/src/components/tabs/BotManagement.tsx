...
export const BotManagement = () => {
  ...
  return (
    <div className="pb-24 w-full max-w-[1000px] mx-auto px-0 md:px-2">
      <motion.div
      ...
      >
        ...
        {/* KPI: Боты / Активны / Подписчики — компактно и без обрезания на мобиле */}
        <div className="mb-6 sm:mb-8 grid grid-cols-3 gap-2 sm:gap-3">
          {[
            { label: "Боты", value: bots.length },
            { label: "Активны", value: activeBots },
            { label: "Подписчики", value: totalUsers },
          ].map((stat) => (
            <div
              key={stat.label}
              className="min-w-0 rounded-[16px] border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-3 sm:px-4 md:px-5 md:py-4"
            >
              <p className="truncate whitespace-nowrap font-accent text-[10px] sm:text-[11px] font-semibold uppercase tracking-tight sm:tracking-[0.12em] text-fg-tertiary">
                {stat.label}
              </p>
              <p className="mt-1 sm:mt-1.5 font-accent text-[20px] sm:text-[22px] md:text-[26px] font-bold leading-none tabular-nums text-[var(--color-foreground)]">
        ...
        {/* Bots List */}
        <div className="space-y-3 sm:space-y-4" ref={menuRef}>
          {bots.map((bot, index) => {
            ...
            return (
              ...
              >
                <div className="relative group bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[18px] p-3.5 sm:p-4 md:p-5 flex flex-col gap-3 md:gap-3.5 transition-all hover:shadow-lg hover:border-[var(--color-primary)]/30">
                  {/* --- HEADER --- */}
                  <div className="flex items-start justify-between gap-3 sm:gap-4">
                    {/* Left: Avatar + Info */}
                    <div
                      className="flex items-start gap-3 sm:gap-4 min-w-0 cursor-pointer"
                      onClick={() => onEditBot(bot.id)}
                    >
                      {/* Avatar */}
                      <div className="relative shrink-0">
                        <div
                          className="w-[42px] h-[42px] md:w-[48px] md:h-[48px] rounded-[13px] md:rounded-[14px] text-white flex items-center justify-center text-[15px] md:text-[16px] font-black shadow-sm"
                      ...
                      <div className="flex flex-col min-w-0 pt-0.5 md:pt-1 flex-1">
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-0.5 md:mb-1 w-full min-w-0">
                          <h3 className="text-[15px] sm:text-[16px] md:text-[18px] font-extrabold text-[var(--color-foreground)] leading-tight truncate">
                            {bot.name}
                          </h3>
                          <AnimatePresence mode="wait" initial={false}>
                            ...
                            >
                              <StatusBadge tone={isActive ? "success" : "neutral"} label={isActive ? "Активен" : "Черновик"} />
                            </motion.span>
                          </AnimatePresence>
                        </div>
                        <span className="text-[12px] sm:text-[13px] md:text-[14px] font-medium text-[var(--color-foreground-secondary)] truncate block w-full">
                  ...
                  <div className="flex items-center justify-between gap-2 sm:gap-3 rounded-[14px] bg-[var(--color-surface-2)] px-3 py-2.5 sm:px-4 sm:py-3">
                    <dl className="flex min-w-0 items-center gap-3.5 sm:gap-5 md:gap-7">
                      <div className="min-w-0">
                        <dt className="text-[10px] sm:text-[11px] font-semibold text-[var(--color-foreground-tertiary)]">Лиды</dt>
                        <dd className="font-accent text-[15px] sm:text-[16px] font-semibold tabular-nums leading-tight text-[var(--color-foreground)]">
                          {bot.usersCount}
                        </dd>
                      </div>
                      <div className="min-w-0">
                        <dt className="text-[10px] sm:text-[11px] font-semibold text-[var(--color-foreground-tertiary)]">Продажи</dt>
                        <dd className="font-accent text-[15px] sm:text-[16px] font-semibold tabular-nums leading-tight text-[var(--color-foreground)]">
                          {bot.sales || 0}
                        </dd>
                      </div>
                      <div className="min-w-0">
                        <dt className="text-[10px] sm:text-[11px] font-semibold text-[var(--color-foreground-tertiary)]">Выручка</dt>
                        <dd className="font-accent text-[15px] sm:text-[16px] font-semibold tabular-nums leading-tight text-[var(--color-success)] whitespace-nowrap">
                    ...
                    {isActive && bot.paymentProvider ? (
                      ...
                      <span className="shrink-0 rounded-md bg-[var(--color-warning-soft)] px-2 py-1 text-[10px] font-bold text-[var(--color-warning)] whitespace-nowrap">
                        Нет кассы
                      </span>
                    ) : null}
                  </div>
                  ...
                  <button
                    onClick={() => onEditBot(bot.id)}
                    className="w-full h-[40px] sm:h-[42px] md:h-[44px] rounded-[12px] bg-[var(--color-surface-2)] text-[var(--color-foreground)] hover:bg-[var(--color-primary-soft)] hover:text-[var(--color-primary)] font-bold text-[13px] sm:text-[14px] flex items-center justify-center gap-2 transition-colors duration-200"
                  >
...
function MiniSparkline() {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="shrink-0 w-16 h-6 sm:w-24 sm:h-8" aria-hidden="true">
