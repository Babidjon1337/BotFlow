...
class BillingProviderUnavailable(RuntimeError):
    """YooKassa could not be reached for mandatory server verification."""
...
async def verify_billing_notification(payload: dict[str, Any]) -> tuple[bool, Any | None]:
    ...
    try:
        local_payment_id = uuid.UUID(str(metadata["saas_payment_id"]))
    ...
    try:
        if payment.get("status") == "canceled":
            user = await mark_saas_payment_failed(
                local_payment_id,
        ...
        return await apply_successful_saas_payment(
            local_payment_id,
            provider_payment_id=str(payment_id),
            amount=amount,
            currency=currency,
