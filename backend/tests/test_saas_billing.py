...
def test_subscription_price_is_990_without_legacy_tariffs():
    """Единственный продукт — подписка бота 990 ₽/мес; тарифа 3000 ₽ больше нет."""
    from config import SAAS_BOT_BASE_PRICE_RUB
    from services.saas_billing import PRODUCTS

    assert SAAS_BOT_BASE_PRICE_RUB == 990
    for key in ("pro", "basic", "subscription"):
        product, amount, _description = PRODUCTS[key]
        assert product == "pro_initial"
        assert amount == 990
    assert "license" not in PRODUCTS


def test_checkout_rejects_amount_below_base_price():
    """Доплаты за функционал только увеличивают сумму, занизить её нельзя."""
    with pytest.raises(saas_billing.BillingError, match="базовой"):
        asyncio.run(saas_billing.create_checkout(1, "pro", amount_rub=500))
