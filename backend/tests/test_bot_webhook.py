...
def test_create_bot_without_token_creates_unpublished_draft(monkeypatch):
    """A draft may exist before Telegram is connected; no webhook is installed."""
    user = SimpleNamespace(id=1, telegram_id=42)
    created_bot = SimpleNamespace(id=18)
    create_config = AsyncMock(return_value=created_bot)

    monkeypatch.setattr(api_router, "get_current_user", AsyncMock(return_value=user))
    monkeypatch.setattr(api_router, "create_user_if_not_exists", AsyncMock(return_value=user))
    monkeypatch.setattr(api_router, "get_user_bots", AsyncMock(return_value=[]))
    monkeypatch.setattr(api_router, "create_bot_config", create_config)
    monkeypatch.setattr(
        api_router.BotApiResponse,
        "from_orm_bot",
        lambda *_: SimpleNamespace(model_dump=lambda **_: {"id": 18, "status": "draft"}),
    )

    result = asyncio.run(api_router.create_bot(_request(), BotCreateApiRequest(displayName="Черновик")))

    assert result == {"id": 18, "status": "draft"}
    create_config.assert_awaited_once_with(
        owner_id=1,
