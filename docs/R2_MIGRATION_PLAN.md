...
Статус: migration-каркас реализован; forward/backfill/downgrade успешно
проверены 27.08.2026 на изолированной PostgreSQL. К реальной БД не применялся.
Дата: 27.08.2026.
...
Этот документ не является миграцией и не меняет runtime-поведение. Реализация
первого малого шага находится в
`backend/database/migrations/versions/f3a1c9d2e8b4_add_bot_lifecycle_contract.py`.
